...
class Parameter {
    ...
    setInitParameter() {
        ...
        const n = 6;
        for (let i = 1; i < n - 1; i++) {
            this.param[i][0] = MyStat.random_norm(ConstVal.MU0[i - 1][0] + d, ConstVal.MU0[i - 1][1]);
            this.param[i][1] = Math.exp(MyStat.random_norm(ConstVal.LOG_SIGMA0[i - 1][0], ConstVal.LOG_SIGMA0[i - 1][1]));
        }
        // Lid + Adr
